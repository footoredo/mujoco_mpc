...
class QuadrupedClimber : public ThreadSafeTask {
 public:
  std::string Name() const override;
  std::string XmlPath() const override;
  class ResidualFn : public mjpc::BaseResidualFn {
   public:
    explicit ResidualFn(const QuadrupedClimber* task, int current_mode = 0)
        : mjpc::BaseResidualFn(task), current_mode_(current_mode) {}
    ...
    void Residual(const mjModel* model, const mjData* data,
                  double* residual) const override;
   private:
    friend class QuadrupedClimber;
    int current_mode_;
  };
  QuadrupedClimber() : residual_(this) {}
  void TransitionLocked(mjModel* model, mjData* data) override;
