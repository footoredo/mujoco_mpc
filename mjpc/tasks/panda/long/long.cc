...
std::string Long::XmlPath() const {
  return GetModelPath("panda/long/task.xml");
}
std::string Long::Name() const { return "Panda Long"; }
// "hand|slidedoorhandle|leftdoorhandle|rightdoorhandle|barleft|barright"
const std::array<std::string, 10> object_names = {
    "hand", "slidedoorhandle", "leftdoorhandle", "rightdoorhandle", "leftcabinetinside", "barleft", "barright", "weight", "weightsensor", "microwave_handle"
};

const std::array<std::string, 4> joint_names = {
    "slidedoorjoint", "leftdoorhinge", "rightdoorhinge", "micro0joint"
};
...
//     Residual (4): control
// ------------------------------------------------------------
void Long::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  residual[counter++] = std::max(joint_target - std::abs(*joint), 0.0);
...
double* AJointByName(const mjModel* m, const mjData* d,
...
void Long::TransitionLocked(mjModel* model, mjData* data) {
  double residuals[100];
  residual_.Residual(model, data, residuals);
  
  double* slide_door_joint = AJointByName(model, data, "slidedoor_joint");
  // double* left_door_hinge = JointByName(model, data, "leftdoorhinge");
  // double* right_door_hinge = JointByName(model, data, "rightdoorhinge");
  double* weight_sensor = SensorByName(model, data, "weightsensor");
  double* weight = SensorByName(model, data, "weight");
  double distance = mju_dist3(weight_sensor, weight);

  if (distance > 0.05) {
    slide_door_joint[0] = 0.0;
  }
}
